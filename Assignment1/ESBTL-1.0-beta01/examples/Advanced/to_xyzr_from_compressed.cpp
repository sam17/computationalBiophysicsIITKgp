...
int main(int argc, char** argv){
  ...
  if (argc != 3 ){
    std::cerr << "Please provide a filename and a file type among {ASCII,BZIP2,GZIP}"  << std::endl;  
    return EXIT_FAILURE;
  }
  ...
  ESBTL::Reading_mode mode = ( mode_str=="BZIP2"?ESBTL::BZIP2:(mode_str=="GZIP"?ESBTL::GZIP:ESBTL::ASCII) );
  switch(mode){
    case ESBTL::BZIP2:
      reading_ok=ESBTL::read_a_pdb_file<ESBTL::BZIP2>(argv[1],sel,builder,Accept_none_occupancy_policy());
      break;
    case ESBTL::GZIP:
      reading_ok=ESBTL::read_a_pdb_file<ESBTL::GZIP>(argv[1],sel,builder,Accept_none_occupancy_policy());
      break;
    default:
      reading_ok=ESBTL::read_a_pdb_file(argv[1],sel,builder,Accept_none_occupancy_policy());
      break;
  };    
